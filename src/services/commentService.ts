...
export class CommentService {
  ...
  async upsertComment(comment: YoutubeComment, videoId: string): Promise<void> {
    ...
    const commentData: Omit<CommentsRow, "first_seen_at" | "published_at"> & {
      ...
      video_id: videoId,
      parent_comment_id: comment.reply
        ? comment.cid.substring(0, comment.cid.lastIndexOf("."))
        : null,
      text: comment.text,
    ...
    if (fetchError) {
      console.error(`Error fetching existing comment ${comment.cid}`, {
        error: fetchError,
      });
      throw fetchError;
    }
    ...
    if (existing) {
      ...
      if (existing.text !== commentData.text) {
        ...
        changes.push({
          comment_id: comment.cid,
          attribute_name: "text",
          old_value: existing.text,
          new_value: commentData.text,
        });
      }
      if (existing.votes !== commentData.votes) {
        ...
        changes.push({
          comment_id: comment.cid,
          attribute_name: "votes",
          old_value: String(existing.votes),
          new_value: String(commentData.votes),
        });
      }
      if (existing.reply_count !== commentData.reply_count) {
        ...
        changes.push({
          comment_id: comment.cid,
          attribute_name: "reply_count",
          old_value: String(existing.reply_count),
          new_value: String(commentData.reply_count),
        });
      }
      if (existing.is_hearted !== commentData.is_hearted) {
        ...
        changes.push({
          comment_id: comment.cid,
          attribute_name: "is_hearted",
          old_value: String(existing.is_hearted),
          new_value: String(commentData.is_hearted),
        });
      }
      ...
      if (Object.keys(updates).length > 0) {
        ...
        if (updateError) {
          console.error(`Error updating comment ${comment.cid}`, {
            error: updateError,
          });
          throw updateError;
        }
        ...
        const { error: updateLogError } = await this.db
          ...
          .insert(changes);
        if (updateLogError) {
          console.warn(`Error inserting comment_updates for ${comment.cid}`, {
      ...
      const { error: insertError } = await this.db.from("comments").insert({
        ...commentData,
        first_seen_at: new Date().toISOString(),
      });
      if (insertError) {
        console.error(`Error inserting comment ${comment.cid}`, {
          error: insertError,
        });
        throw insertError;
      }
  ...
  async processCommentBatch(
    ...
    for (const comment of comments) {
      try {
        ...
        console.error(
          `Failed to process comment ${comment.cid} for video ${videoId}:`,
          error,
        );
